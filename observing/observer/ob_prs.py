...
def find_open_merged_pr(previous_state, current_state, main_repo):
    ...
    # Validate previous and current states
    prev_prs = previous_state.get('prs', {})
    curr_prs = current_state.get('prs', {})
    ...
    for pr_number, curr_state in curr_prs.items():
        if pr_number not in prev_prs:
            if curr_state == 'open':
                ...
                if pr is not None and pr.merged:  # Ensure pr exists before checking merged status
